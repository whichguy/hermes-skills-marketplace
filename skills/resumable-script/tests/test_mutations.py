...
def test_blob_corruption():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with open(flow_path, "w") as f: f.write(FLOW_SRC)
        ...
        subprocess.run([sys.executable, ENGINE, "run", "--flow", flow_path, "--state-dir", td], env=env, capture_output=True)
        
        # 2. Corrupt the blob
        blobs_dir = os.path.join(td, "blobs")
        blob_file = os.path.join(blobs_dir, os.listdir(blobs_dir)[0])
        with open(blob_file, "w") as f: f.write('{"corrupted": true}')
        
        # 3. Second run - should fail with exit 3
        res = subprocess.run([sys.executable, ENGINE, "run", "--flow", flow_path, "--state-dir", td], capture_output=True, text=True, env=env)
        if res.returncode == 3:
...
def test_schema_version():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with open(jp, "w") as f:
            f.write(json.dumps({"v": 999, "seq": 0, "type": "run_started", "run_id": "R", "flow_id": "blob_test"}) + "\n")
...
def test_torn_tail_recovery():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        jp = os.path.join(td, "journal.jsonl")
        with open(jp, "w") as f:
            f.write(json.dumps({"v": 1, "seq": 0, "type": "run_started", "run_id": "R", "flow_id": "blob_test", "engine": "py", "input": None}) + "\n")
