...
metadata:
  hermes:
    ...
    config:
    - key: maps.enabled
      description: Enable maps skill behavior
      default: true
      prompt: Enable maps skill?
---
---
...
- **Uber deep links & address resolution** → `check-in-automation` skill (canonical). The maps skill provides geocoding (`search`, `reverse`) as a building block; the check-in-automation skill owns the full Uber deep link generation, known_places.json resolution chain, and calendar event patterns.
- **Check-in reminders & airline APIs** → `check-in-automation` skill (canonical). The maps skill provides location resolution for destinations; the check-in-automation skill owns the 5-layer SPA discovery methodology, checkin_links.json registry, and calendar event templates.
- **Calendar event hyperlink patterns** → `messaging-platform-formatting` skill (canonical). HTML `<a href>` patterns for Gmail booking email links, Maps address links, and check-in page links in Google Calendar event descriptions.
