...
**Pitfall:** avoid running `hermes status --all` in a chat/tool context unless you have verified redaction, because some versions have printed provider tokens too verbosely in the API-key/auth section. Prefer `hermes config`, `hermes gateway status`, or targeted status commands when preparing user-visible output.
